...
Blocarea operațiilor:
 ...
 Dacă procesul de citire încearcă să citească din pipe atunci când nu există date, acesta se blochează (așteaptă) până 
când sunt disponibile date.
În mod similar, dacă procesul de scriere încearcă să scrie atunci când pipe-ul este plin, acesta se blochează până când
există spațiu liber.
Detectarea sfârșitului datelor:

 Atunci când capătul de scriere este închis de toate procesele care îl folosesc, orice proces de citire primește un semnal 
EOF (End of File) atunci când nu mai sunt date de citit.
...
Când un proces creează un pipe și apoi folosește fork() pentru a crea un proces copil, ambele procese vor avea acces la 
aceleași capete ale pipe-ului.
Prin închiderea capetelor neutilizate (de exemplu, copilul închide capătul de citire și părintele capătul de scriere), 
se delimitează direcția de comunicare și se evită potențiale blocaje sau confuzii.
Exemplu simplu de flux de date printr-un pipe
Să presupunem că avem un proces părinte și un proces copil care folosesc un pipe pentru a transfera date de la copil la 
părinte:
...
Capătul de scriere pipe_fd[1] este închis, iar părintele citește date din pipe_fd[0].
 ...
 Când copilul termină de scris datele, închide pipe_fd[1]. Părintele va citi datele și, la final, va primi EOF, indicând 
că nu mai sunt date.
Astfel, pipe-ul oferă un canal simplu de comunicare între procese, permițând transmiterea de date cu sincronizare automată
a citirii și scrierii.
*/
